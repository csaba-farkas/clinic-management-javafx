 ...
 * @author Csaba Farkas
 */
public class AddNewAppointmentSceneController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        /**
         * Set up action handler on 'Create' button.
         */
        this.createButton.setOnAction(event -> {
            ...
            try {
                ...
                if(!errorMessage.isEmpty()) {
                    ...
                    //Call the API to create the new Appoitnment
                    appointment = AppointmentClient.createAppointment(appointment);
                    Alert alert = new Alert(AlertType.INFORMATION, "Appointment was created successfully.");
                    alert.initStyle(StageStyle.UTILITY);
                    alert.show();
                    resetForm();
                ...
                Alert alert = new Alert(AlertType.ERROR, iie.getMessage());
                alert.initStyle(StageStyle.UTILITY);
                alert.show();
            } catch (Exception e) {
                e.printStackTrace();
    ...
    /**
     * Worker method to clear all data from the form and reset it's values.
     */
    private void resetForm() {
        ...
        if(this.isExistingPatient.isSelected()) {
            ...
            if(this.patientSearchCombo.getSelectionModel().getSelectedIndex() > -1) {
                this.patientSearchCombo.getSelectionModel().clearSelection();
            }
            this.patientSearchCombo.setDisable(true);
        }
        this.firstNameField.clear();
